...
class InputClassTests(TestCase):
    ...
    def test_valid_bound_field(self):
        form = ExampleForm({'name': 'Alan', 'password': '123'})
        self.assertEqual('form-control is_valid', input_class(form['name']))
        self.assertEqual('form-control ', input_class(form['password']))

    def test_invalid_bound_field(self):
        form = ExampleForm({'name': '', 'password': '123'})
        self.assertEqual('form-control is_invalid', input_class(form['name']))